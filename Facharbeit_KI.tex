...
% Ab "Auswertung" weiter Duden nutzen
% Vorwärtsgerichtetes Neuronales Netzwerk groß oder klein?
...
So weckte das Thema mein Interesse, mich damit zu beschäftigen, da das faszinierend ist zu verstehen, wie die Entscheidungsverhalten
ablaufen nur anhand der Mathematik. Auch ist die Facharbeit eine Motivation für mich, für den Einstieg in das Themenfeld KI, da ich
an das Bundeswettbewerb-KI teilnehmen möchte. Ziel der Facharbeit ist es, eine KI zu programmieren, die handgeschriebene Zahlen mit
einer Wahrscheinlichkeit von mindestens 97\% bestimmen zu können.
...
Denn je größer der Datensatz ist, desto besser schneidet mehrschichtiges Lernen ab als maschinelles Lernen. Im Gegenzug verbraucht diese Methode mehr
...
Datensätze sind für die KIs das wichtigste Werkzeug für das Lernen. Denn die Qualität des Datensatzes bestimmt die Lerneffizienz der KI.\@
Je schlechter die Qualität ist, desto schlechter lernt auch die KI.\@ Wichtige Merkmale für Qualität sind die Auflösung, die Anzahl der Daten und die Variabilität.
...
weiterzuleiten, falls das Signal ankommen sollte. Damit das Signal ankommt, braucht es die dritte Komponente zwar das Axon. Sie wird als Leitungsbahn
...
negative Werte annehmen kann, was bei den Neuronen nicht der Fall ist, da das Axon nur das Signal weiterleiten oder abbrechen kann. Sie können auch mit 0 und 1 bezeichnet werden.
\begin{figure}[h]
    \centering
...
Lernprozess der KI auf das Wichtigste begrenzt wird. Welche Merkmale eines Bildes gemeint sind, wird im Kapitel \nameref{feature} verdeutlicht.
...
Die Extrahierung wird durch 2D Konvolution berechnet. Im zweiten Bereich werden die
...
Dafür wird die Fehlerzurückführung (Backpropagation) verwendet. Erstmal werden nur die partiellen Ableitungen berechnet. Warum sie gebraucht werden, wird im Kapitel \nameref{back} genauer
...
Auch die Funktion $C(\vec{y},\vec{x}^{(A,1)})$ wird in Kapitel \nameref{lost} genauer erläutert.
...
\end{figure}
Zwei weitere wichtige Operationen sind der Max-Pooling und der Flattening. Aufgabe von Max-Pooling ist es, unnötige Merkmale
...
\end{equation}
Der Kernel kann nach der Abbildung~\ref{kernel} verschiedene Formen von Matrizen annehmen, dadurch entsteht durch die 2D Konvolution eine neue Matrix, in dem Fall ein neues Bild.
...
Da sowohl die Eingabe des Bildes sowie der Kernel beide Matrizen sind, kann mit der 2D Konvolution gerechnet werden. Sie ist
...
Werden diese Eigenschaften hinzugefügt, folgt daraus für die 2D Konvolution:
\begin{equation}
...
durch die einzelnen Regionen des Bildes durchgegangen wird und der höchste Wert von den 4 Pixel ausgewählt wird:
\begin{equation}
...
\end{equation}
Bei der Backpropagation ist der Wert der Verlustfunktion nur von den größten Werten der einzelnen Regionen abhängig, da nur diese an der nächsten Schicht weitergegeben wurden.
...
der vorhergesagten Zahl $\vec{x}^{(A,1)}$ mit der Ausgangszahl $\vec{y}$ berechnet wird. Je größer die Abweichung ist, desto unpräziser trifft die KI Vorhersagen. Für die
...
der KI sehr hoch sein kann. Es wird von mehrere Tausende Parameter gesprochen. So ist die Berechnung der Parameter uneffizient und könnte für einen
...
\end{equation}
Hier wurde ein neuer Parameter $\eta$ hinzugefügt, diese nennt sich Lernrate. Sie gibt an, wie schnell sie sich an das Minimum antasten soll.
Es ist wichtig, einen guten Wert für die Lernrate zu setzen, denn falls die Lernrate zu groß wird, kann sie vielleicht über das Minimum vorbeispringen.
...
für 70000 Bilder nur 700 Bilder berechnet. Das Antasten des Minimums ist nicht sehr präzise, aber dennoch aussagekräftig und zeitsparend. Die
...
Da die Werte bei jeder Schicht weitergegeben wurden, kann die Kettenregel angewendet werden:
\begin{equation}
...
Internet anbietet. So müssen nicht die einzelnen Elemente der Vektoren ausgerechnet werden, sondern kann alle Elemente des Vektors auf einmal berechnen.
...
Mehr dazu im Kapitel \nameref{auswertung}. Für die Architektur des Netzwerkes werden mehrere Schichten von 2D Konvolution sowie Schichten von Max Pooling angesetzt.
