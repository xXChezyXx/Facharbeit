...
% Quellen einfügen
% Schluss, Auswertung zu Ende schreiben
...
was leicht zu erkennen ist. Wenn aber der Vorgang versucht wird zu erklären, so wird es klar, dass es nicht ganz einfach ist
...
\end{figure}
Eine Problemstellung ensteht, wie handgeschriebene Zahlen erkannt werden können.
...
Außerdem wird die Funktionsweise eines Textscanners verstanden, was im Alltag von den meisten unbewusst benutzt wird.
Viele Forscher versuchten deshalb in der Vergangenheit nach Problemlösungen zu suchen. Ein Ansatz wäre das Herausfinden
der Strukturen der einzelne Zahlen und damit die Unterscheidungen zwischen den Zahlen.
...
komplexe Strukuren erkennen kann, was ein Mensch vielleicht nicht erkennt, wird die Facharbeit für die Erkennung der handgeschriebenen Zahlen eine KI programmieren. 
Ziel der Facharbeit ist es eine KI zu programmieren, die handgeschriebene Zahlen mit einer Wahrscheinlichkeit von mindestens 97\% erkennen zu können.
...
auch verschiedene Arten von KIs, welche Funktion jede einzelne Art besitzt, erläutert. Die angewendete Lernmethode wird genauer erläutert.
Danach wird in Kapitel 3 über mehrschichtiges Lernen erläutert und Voraussetzungen für diese Methodik genannt. Im Anschluss wird in Kapitel 4
der Aufbau eines faltendes neuronales Netzwerk dargestellt und die Funktionsweise mathematisch erläutert. Dabei wird darauf eingegangen, wie eine KI
...
Mehr dazu im Kapitel \nameref{auswertung}. Für die Architektur des Netzwerkes wird eine 2D Konvolutionsschicht mit einem Kernel einer Dimension von 3$\times$3, darauffolgend eine Max-Pooling Schicht.
An der Konvolutionsschicht wird die Aktivierungsfunktion ReLU angehängt. Nach der Max-Pooling Schicht wird das zum Vektor abgeflacht und mit zwei weiteren neuronale Schichten verbaut.
\begin{figure}[h]
    \centering
    \includegraphics[width=400pt, keepaspectratio]{images/cnn}
    \caption[Architektur eines CNNs]{Insgesamt 6 Schichten, wobei die 2D Konvolutionsschicht eine Dimension von 26$\times$26 besitzt mit der Tiefe von 5.
